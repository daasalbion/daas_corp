...
//WAP V1.1
class WapController extends Zend_Controller_Action
{
    var $logger;
    var $ua = array();
    var $msisdn;
    ...
    public function init()
    {
        ...
        $this->logger->info('INIT WAP PRUEBA - ENTERMOVIL');
        ...
        if( !isset( $namespace->ancho ) && empty( $namespace->alto ) ){
            ...
            if( !is_null( $alias ) ){
                ...
                $id_promocion = $this->alias[$alias];
                $this->logger->info( 'id_promocion: ' . $id_promocion );

                $this->_analizarSolicitud( $id_promocion );

                if( isset( $this->tipo_mensaje ) ){

                    $parametros['alias'] = $alias;
                    $this->_forward( 'manejador', 'wap', null, $parametros );
                }
            }else{

                //van a ser tipo 3 y 4
                $this->logger->info( "mierda" );
                $this->logger->info( "mirar: " . print_r( $datos_user_agent, true ) );
                $this->ua = $datos_user_agent;
                $this->_analizarSolicitud( );
                
                $this->logger->info( 'tipo de mensaje: ' . $this->tipo_mensaje );

                if( isset( $this->tipo_mensaje ) ){

                    $parametros['alias'] = 'PORTAL';
                    $this->_forward( 'manejador', 'wap', null, $parametros );
                }
                //exit;
            }
    ...
    private function analizarSolicitud(){
        ...
        if( ( $this->solicitudValida == false )  ){
            ...
            if( $this->ua['is_mobile'] ){
                ...
                $this->_redirect('/wap/manejador-de-mensajes/tipo/1');
            }else{
                //es una pc
                $this->_redirect('/wap/manejador-de-mensajes/tipo/2');
            }
        }
    }
    ...
    public function indexAction() {
        ...
        if($this->solicitudValida == false){
            ...
            if( !is_null( $alias ) ){
                ...
                //1 dia
                $namespace->setExpirationSeconds(60*60*60);
    ...
    public function altaSuscriptoAction(){
        ...
        if( !is_null( $parametros ) ){
            ...
            if( $this->_estaSuscripto( $datos ) ){
                ...
                $this->_redirect( '/wap/activacion/id-promocion/' . $datos['id_promocion'] .'/suscripto/false' );

            }else{

                $this->_redirect( '/wap/activacion/id-promocion/' . $datos['id_promocion'] .'/suscripto/true' );
            }
    ...
    public function siglasAction() {
        ...
        $path_archivo_descarga = $this->_generarFondoPersonalizado( $path_fondo, $iniciales );
    ...
    private function _generarFondoPersonalizado($path_fondo, $iniciales) {

        $this->logger->info( 'generarFondoPersonalizado' );
        ...
        if( file_exists( $path_final ) ) {//Si existe en /cache
            ...
            $this->logger->info( 'path_temporal: ' . $path_fondo_temporal );
            ...
            $this->logger->info( 'path_origen: ' . $path_fondo );
            $this->logger->info( 'path_destino: ' . $path_final );
            ...
            if( file_exists( basename( $path_final )  )) {
                ...
                $this->logger->info('generarFondoPersonalizado() -> NO SE CREO EL ARCHIVO!!!');
                $this->logger->info('intentando de nuevo!!!');
                //unset( $salida[0] );
                ...
                if( !$estaSuscripto ){//NO esta suscripto...
                    ...
                    if($datos['id_carrier'] == 2) {//TIGO
                        ...
                        if($secuencia_wap_request > 0) {
                            ...
                            $url_entermovil = 'http://www.entermovil.com.py/wap/tigowapfw/status/$status$/request_id/$request_id$';
    ...
    public function manejadorAction(){
        ...
        $this->logger->info( '--->MANEJADOR_ACTION' );
        $parametro = $this->_getParam( 'alias', null );
        $this->logger->info( 'parametro: ' . $parametro );
        //seteando la promocion de acuerdo al alias
        $this->_helper->_layout->setLayout( $this->layouts[$this->alias[$parametro]] );
        ...
        switch ( $this->tipo_mensaje ) {
            ...
            case 2:
                ...
                $this->view->mensaje = array(
                    'Active la conexión 3G de su celular',
                    'y pulse: ',
                    '<a href="'."http://". $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'].'">RECARGAR</a>',
                );
                if( $this->alias[$parametro] == '72' ){

                    $this->_cargarImagenes2( $this->ua['ancho'] );

                }else if ( $this->alias[$parametro] == '77' ){

                    $this->_cargarImagenes( $this->ua['ancho'] );
                }

                $this->view->ua= $this->formato;

                //setear errores
                $info_error['id_promocion'] = $this->alias[$parametro];
                $info_error['error'] = 'SESION EXPIRADA';
                $info_error['linea'] = 90;
                $info_error['alias'] = $parametro;
                $info_error['direccion_ip'] = $_SERVER['REMOTE_ADDR'];
                $this->_logearErrores( $info_error );
                //mostrar vista apropiada
                $this->_helper->viewRenderer('mensaje-error');
            ...
            case 3:
                ...
                $this->view->mensaje = array(
                    'Favor presionar:',
                    '<a href="'."http://". 'www.entermovil.com.py/' . $parametro.'">RECARGAR</a>',
                );
    ...
    private function _analizarSolicitud( $id_promocion = null ){

        $this->logger->info( '_analizarSolicitud' );

        //$this->_procesarUserAgent();

        if( isset( $this->ua['ancho'] ) ){
