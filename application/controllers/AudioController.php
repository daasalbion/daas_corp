...
class AudioController extends Zend_Controller_Action
{
    ...
    public function ____BKP___indexAction()
    {
        ...
        if(is_null($this->msisdn)) {
            ...
            if($id_promocion > 0) {
                ...
                if($cel == $this->_getFormatoCorto($this->msisdn)) {
                    ...
                    $this->view->proveedor = 'ENTERMOVIL';
                    $this->view->servicio = 'INGLES';
                    $this->view->descripcion = 'Lecciones de Inglés';
                    $this->view->mensaje = $this->info_contenido['mensaje'];
                    $this->view->url_descarga = 'http://www.entermovil.com.py/audio/descargar/?k='.$auth;
                    $this->view->nombre_archivo = $info_audio['audio'];
    ...
    private function _consulta($accion, $datos) {
        ...
        if($accion == 'GET_AUDIO_CONTENIDO') {
            $sql = "SELECT * FROM promosuscripcion.audio_x_contenido WHERE id_contenido = ? AND id_promocion = ?";
            $rs = $db->fetchRow($sql, array(
                $datos['id_contenido'],
                $datos['id_promocion']
            ));
